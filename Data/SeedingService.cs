...
using SalesWebMvc.Models; // Mesma biblioteca da classe SalesWebMvcContext
using SalesWebMvc.Models.Enums;

namespace SalesWebMvc.Data
{
    // Classe criada para popular as tabelas nos campos de dados SalesWebMvc

    public class SeedingService
    {
        // Registrando dependencia do SeedingService com o DbContext SalesWebMvcContext
        private SalesWebMvcContext _context;

        // Criando construtor que recebe o objeto SalesWebMvcContext para indicar que a injeção
        // de dependencia vai acontecer
        public SeedingService(SalesWebMvcContext context)
        {
            // _context da classe recebe context como parametro
            _context = context;

        }

        // Obs: Quando um Seeding Service for criado, automaticamente ele vai receber uma instancia do
        // context tambem


        // Metodo responsável de popular o banco de dados
        public void Seed()
        {
            // Any do Ling testa se existe algum registro na tabela Departament ou Seller ou SalesRecord

            if (_context.Departament.Any() || 
                _context.Seller.Any() || 
                _context.SalesRecord.Any() )
            {
                return; // Corta a execução do metodo. Banco de dados ja foi populado.
            }

            // Instancia os objetos, adiciona nos campos Id e Name, e salva no banco de dados
            // Criando 4 departamentos
            ...
            // Criando Vendedores
            // d1 associação entre objeto "Computers"
            ...
            // Adicionando os objeto da classe Departament: adicionando departamentos
            _context.Departament.AddRange(d1, d2, d3, d4);

            // Adicionando os objetos da classe Seller: adicionando vendedores
            _context.Seller.AddRange(s1, s2, s3, s4, s5, s6);

            // Adicionando os objetos da classe SalesRecord: adicionando vendas
